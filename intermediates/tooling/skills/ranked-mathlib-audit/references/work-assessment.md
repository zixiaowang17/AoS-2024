...
| Small adaptation (yellow) | The core mathematics exists; a thin wrapper, representation conversion or a bounded local compatibility proof remains. | Identify the existing construction and the specific conversion or connecting obligation. |
...
Converting between equivalent representations, introducing a convenient wrapper, or proving
a local identity from existing results is usually yellow. Explain the bridge; calling a proof
