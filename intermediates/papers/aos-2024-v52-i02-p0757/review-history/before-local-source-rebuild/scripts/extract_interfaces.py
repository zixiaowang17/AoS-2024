...
\]
''',[16],'Section 6.1 — permutation distribution and critical value, equations (27)-(28)',{'D15':'The pooled observations are the two original samples.','D17':'Each permuted replicate evaluates the weighted statistic on the regrouped observations.'},context='we estimate $q_\alpha$ using a random permutation scheme in practice, as follows.',phrases=['equations (27)-(28)'],symbols=[r'\widehat\Gamma_{nm}(t)',r'\widehat q_\alpha'],shape='Monte Carlo permutation CDF from K independent uniform permutations, and its upper-alpha generalized inverse. The second displayed entry of Y_Pi_j is literally V_Pi_j(2), not V_Pi_j(n+2); this inconsistent split is preserved and separately flagged. Re-evaluation is expressed as the same statistic functional, not a fixed-weight shortcut.')
