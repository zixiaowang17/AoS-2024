"""Extract original Chapter 3 spectral-EL prerequisites; all appendices remain excluded."""
import json
...
''',[106],'Section 3.4 — spectral moment vector',symbols=[r'\mathcal M_\theta'],shape='General parameter-dependent spectral moment vector, using script M. It need not be the zero vector or equal theta. Its relationship to the estimating functions and true density is a separate moment condition.')
...
''',[108],'Section 3.5.1 — full-data periodogram',{'D1':'The observed time stretch comes from the real-valued process in the common spectral framework.'},symbols=[r'I_n(\lambda)'],shape='Uncentered squared Fourier transform of the full observed stretch, normalized by 2*pi*n. The imaginary unit is distinct from the block index i. The empirical sums later omit the zero Fourier frequency.')
add('D6','empirical spectral average',r'''
Using $I_n(\cdot)$ in place of $f(\cdot)$, a standard empirical spectral average for estimating a spectral mean $\int_{-\pi}^\pi G_\theta(\lambda)f(\lambda)d\lambda$ from (3.2) is given by
\[
T_n(\theta)\equiv\frac{2\pi}n\sum_{|j|=1}^{\lfloor n/2\rfloor}G_\theta(\lambda_{j,n})I_n(\lambda_{j,n}),\qquad\theta\in\Theta,\tag{3.5}
\]
...
''',[108],'Section 3.5.1 — full-data empirical spectral average (3.5)',{'D2':'The Riemann sum weights the periodogram by G_theta.','D5':'It uses the full-data periodogram I_n.'},symbols=[r'T_n(\theta)',r'\lambda_{j,n}'],shape='Riemann sum over both signs of every nonzero Fourier frequency up to floor(n/2). For even n the printed range includes both -pi and pi. It is the full-data average, distinct from the non-overlapping block averages used to form SEL.')
add('D7','non-overlapping (NOL)',r'''
For a block length $b<n$, the data are divided into $N\equiv\lfloor n/b\rfloor$ non-overlapping (NOL) blocks/subsamples of length $b$ as $(X_{1+(i-1)b},\ldots,X_{ib})$, $i=1,\ldots,N$. The periodogram from the $i$th subsample, denoted as
\[
I_{i,\mathrm{NOL}}(\lambda)\equiv\frac1{2\pi b}\left|\sum_{s=1}^b X_{(i-1)b+s}e^{-\imath\lambda s}\right|^2,\qquad\lambda\in[-\pi,\pi],
\]
''',[108,109],'Section 3.5.1 — non-overlapping blocks and periodograms',{'D1':'The blocks partition the observed time stretch of the process, up to the unused remainder after N full blocks.'},symbols=[r'N\equiv\lfloor n/b\rfloor',r'I_{i,\mathrm{NOL}}(\lambda)'],shape='N=floor(n/b) disjoint length-b blocks with their own periodograms, normalized by 1/(2*pi*b). The original construction uses only N full blocks; it does not require b to divide n.')
add('D8','empirical spectral average',r'''
is used for a subsample-analog of the empirical spectral average $T_n(\theta)$, $\theta\in\Theta$, from (3.5) as
\[
...
\]
where $\lambda_{j,b}\equiv2\pi j/b$, $1\le|j|\le\lfloor b/2\rfloor$; thus, $I_{i,\mathrm{NOL}}$ and $b$ from blocks replace $I_n$ and $n$ in (3.5).
''',[109],'Section 3.5.1 — non-overlapping subsample spectral averages',{'D2':'The block-periodogram Riemann sum uses G_theta.','D7':'The periodogram and count of blocks are the non-overlapping construction.'},symbols=[r'T_{i,\mathrm{NOL}}(\theta)'],shape='Block-scale spectral averages over the non-overlapping periods. Mention of full-data T_n describes the analogous construction; the numerical value of T_n is not required to compute these block averages.')
...
''',[111],'Assumption 3.5.3 — fourth-order cumulant density definition',{'D1':'The cumulants are those of the real-valued time process in the spectral setup.'},symbols=[r'f_4(\lambda_1,\lambda_2,\lambda_3)'],shape='Triple Fourier series of fourth-order cumulants with factor (2*pi)^{-3} and negative imaginary exponent. The standard cumulant notion is ambient; Assumption 3.5.1 supplies the moments in theorem applications. No Gaussian or independent-innovation simplification is inserted.')
...
\end{aligned}\tag{3.9}
\]
''',[111],'Assumption 3.5.3 — spectral covariance nonsingularity',{'D1':'The first contribution uses the squared process spectral density.','D2':'Both covariance contributions use the vector G at the true parameter.','D14':'The second contribution uses the fourth-order cumulant density at (lambda1,lambda2,-lambda2).'},kind='assumption',symbols=[r'V_{\theta_0}',r'V_{\theta_0,2}'],phrases=['positive definite'],shape='Positive definiteness of the sum V_theta0, not separate positive definiteness of its two contributions. Both terms and the reflected-frequency vector are retained. The second contribution need not vanish for the allowed nonlinear processes.')
add('D16','spectral M-estimator',r'''
Based on $T_n(\theta)$ and the spectral moment condition (3.2), we define a spectral M-estimator $\widehat\theta_n$ as the solution to
\[
T_n(\theta)=\mathcal M_\theta.\tag{3.10}
\]
''',[114],'Section 3.6.1 — spectral M-estimator (3.10)',{'D6':'The estimating equation uses the full-data spectral average.','D3':'Its right-hand side is the parameter-dependent spectral moment vector.'},symbols=[r'\widehat\theta_n',r'T_n(\theta)=\mathcal M_\theta'],shape='Root of the full-periodogram estimating equation. Theorem 3.6.1 asserts existence of a consistent asymptotically normal solution sequence, not uniqueness or those properties for every root.')
...
''',[117],'Section 3.6.2 — overlapping blocks and their printed periodogram',{'D1':'The overlapping windows are taken from the observed process.'},symbols=[r'I_{i,\mathrm{OL}}(\lambda)'],shape='All n-b+1 consecutive length-b overlapping blocks. The printed periodogram prefactor is 2*pi/b, unlike the NOL prefactor 1/(2*pi*b). Preserve the original formula and flag its normalization mismatch.',note='PDF 117 literally prints (2π/b) before the squared Fourier sum; PDF 108–109 uses 1/(2πb) for the non-overlapping periodogram. The factor differs by 4π². No silent correction or claim that the two normalizations agree is made.')
...
\]
''',[117],'Section 3.6.2 — overlapping subsample spectral average',{'D2':'The overlapping-periodogram sum is weighted by the spectral estimating functions at its argument.','D18':'It uses the overlapping block periodogram and its printed normalization.'},symbols=[r'T_{i,\mathrm{OL}}(\widehat\theta_n)'],context=r'Using the spectral M-estimator $\widehat\theta_n$ in place of the true parameter $\theta_0$ (Section 3.6.1), define a collection of overlapping (OL) subsample statistics from the data $X_1,\ldots,X_n$ as',shape='Overlapping-block spectral-average function, first displayed at the fitted parameter. Section 3.8.2 explicitly reuses the same block statistic as a function of every theta; that function itself does not require fitting theta. The evaluated collection is recorded separately.')
members['D19']['application_context']=[dict(text=r'For given value of $\theta$, we then create a collection of overlapping (OL) periodogram sub-averages $\{T_{i,\mathrm{OL}}(\theta),\theta\in\Theta\}_{i=1}^{n-b+1}$, noting that the $i$th OL data block $(X_i,\ldots,X_{i+b-1})$ of length $b$ determines $T_{i,\mathrm{OL}}(\theta)$ as a function of $\theta$.',evidence=[dict(page=128,location='Section 3.8.2 — block statistic as a function of theta')])]
add('D20','overlapping (OL) subsample statistics',r'''
Using the spectral M-estimator $\widehat\theta_n$ in place of the true parameter $\theta_0$ (Section 3.6.1), define a collection of overlapping (OL) subsample statistics from the data $X_1,\ldots,X_n$ as
\[
\mathcal C_{\mathrm{OL}}\equiv\{T_{i,\mathrm{OL}}(\widehat\theta_n):i=1,\ldots,n-b+1\},
\]
''',[117],'Section 3.6.2 — fitted overlapping-block collection',{'D19':'The collection consists of the overlapping spectral-average statistics.','D16':'Each statistic is evaluated at the spectral M-estimator.'},symbols=[r'\mathcal C_{\mathrm{OL}}'],shape='Empirical collection indexed by all overlapping windows and evaluated at the full-data fitted parameter. Repeated values retain their block multiplicities when sampled; the braces do not justify deduplicating the empirical collection.')
add('D21','iid draws',r'''
We then take $N$ iid draws from the OL collection $\mathcal C_{\mathrm{OL}}$, as $\{T_{i,\mathrm{OL}}^*(\widehat\theta_n)\}_{i=1}^N$, to serve as bootstrap counterparts to $\{T_{i,\mathrm{NOL}}(\theta_0)\}_{i=1}^N$,
''',[117],'Section 3.6.2 — resampling the fitted overlapping collection',{'D20':'The draws are from the fitted overlapping-block collection.','D7':'The number of independent draws is the count N=floor(n/b) of non-overlapping blocks.'},symbols=[r'\{T_{i,\mathrm{OL}}^*(\widehat\theta_n)\}_{i=1}^N'],shape='N conditionally independent draws from the empirical overlapping collection, retaining block multiplicities. Uses the same block length as SEL. The sampled objects are spectral averages, not a reconstructed time series.')
...
\]
''',[128],'Section 3.8.2 — SEL profiling for a smooth function parameter',{'D9':'The profile maximizes the original SEL function over the fiber of h.'},symbols=[r'L_n(\vartheta)',r'\ell_n(\vartheta)'],shape='Profile likelihood and log-ratio for h(theta), using the original max rather than replacing it by sup. Theorem 3.8.1 specifies h:R^p→R^s; its Jacobian rank supplies chi-square degrees of freedom. Source prose has inconsistent dimensions, recorded separately.')
add('D24','estimator',r'''
To approximate the distribution of $\ell_n(\vartheta_0)$ with the SELB procedure, we initially compute an estimator $\widehat\vartheta_n\equiv h(\widehat\theta_n)$ using the smooth function $h(\cdot)$ of a spectral M-estimator $\widehat\theta_n$ for $\theta$ from (3.10), where $\widehat\vartheta_n$ then plays the role of the true parameter $\vartheta_0$ in the bootstrap.
''',[128],'Section 3.8.2 — fitted smooth function parameter',{'D16':'The smooth function is applied to the full-data spectral M-estimator.'},symbols=[r'\widehat\vartheta_n\equiv h(\widehat\theta_n)'],shape='Plug-in smooth function of the spectral M-estimator. The function h is a theorem-local input, and no extra profile optimization defines this fitted target.')
add('D25','OL block statistics',r'''
For given value of $\theta$, we then create a collection of overlapping (OL) periodogram sub-averages $\{T_{i,\mathrm{OL}}(\theta),\theta\in\Theta\}_{i=1}^{n-b+1}$, noting that the $i$th OL data block $(X_i,\ldots,X_{i+b-1})$ of length $b$ determines $T_{i,\mathrm{OL}}(\theta)$ as a function of $\theta$. By independently resampling $N\equiv\lfloor n/b\rfloor$ OL block statistics from this collection, we obtain a bootstrap version $\{T_{i,\mathrm{OL}}^*(\theta),\theta\in\Theta\}_{i=1}^N$ of the non-overlapping (NOL) periodogram sub-averages $\{T_{i,\mathrm{NOL}}(\theta),\theta\in\Theta\}_{i=1}^N$ that build $L_n(\theta)$. This produces a bootstrap rendition $L_n^*(\theta)$ of $L_n(\theta)$, $\theta\in\Theta$,
''',[128],'Section 3.8.2 — resampling entire block-statistic functions',{'D19':'Each resampled object is the overlapping statistic as a function of theta, explicitly reusing the earlier construction.','D7':'The resampling count is N=floor(n/b).','D9':'The SEL objective is applied to the resampled block-statistic functions to define the likelihood process.'},symbols=[r'\{T_{i,\mathrm{OL}}^*(\theta),\theta\in\Theta\}_{i=1}^N'],shape='Resample whole theta-indexed block-statistic functions with shared block indices across theta, then form the SEL likelihood process. This is not independent resampling afresh at every theta and does not depend on the single fitted-parameter collection in Section 3.6.2.')
