...
def main():
    ...
    for n in [1,6,8,9,15,16,22,23,24]:doc[n-1].get_pixmap(matrix=fitz.Matrix(1.6,1.6)).save(ROOT/'evidence'/f'page-{n:02}.png')
    claims=[]
    ...
    manifest=REPO/'corpus/aos/2024/local-pdf-manifest.json'
    source=next(x for x in json.loads(manifest.read_text())['papers'] if x['paper_id']==PID);assert source['sha256']==SHA
