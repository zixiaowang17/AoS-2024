...
members['D27']['statement_original'] = "For all $y'\\in P(M,s)$, the LASSO optimality conditions imply that the LASSO solution is locally linear, namely\n\\[\n\\hat\\beta(y')=\\beta_{(M,s)}(y'):=(X_M^\\top X_M)^{-1}(X_M^\\top y'-\\lambda s).\n\\]\nNote that, while $\\beta_{(M,s)}(y')$ is equal to the LASSO solution for $y'\\in P(M,s)$, it can be computed for $y'\\notin P(M,s)$."
members['D12']['highlight_symbols'] = ['\\hat\\gamma=\\operatorname*{argmax}_{\\gamma\\in[m]}']
