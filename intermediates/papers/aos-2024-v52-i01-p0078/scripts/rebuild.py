"""Rebuild the reviewed StarTrek census without modifying its audit or source evidence."""
import argparse,hashlib,json
from pathlib import Path
import save_inventory,extract_interfaces,finalize_paper
...
def main():
 ...
 for module in [save_inventory,extract_interfaces,finalize_paper]:module.ROOT=out
 save_inventory.main();finalize_paper.main()
 comparisons=[]
 ...
 record=dict(paper_id=ROOT.name,output_directory=str(out),comparisons=comparisons,source_review_performed_by_rebuild=False,note='Regenerates the original reviewed TeX extraction and reruns finalization. Source PDF resolves from the fixed local register; source reviews and original evidence are unchanged.')
 ...
 print('All six StarTrek content artifacts reproduced exactly.')
