...
ALLOWED_SUFFIXES={'.py','.md','.json','.html','.css','.js','.yaml','.yml','.txt','.lean'}
...
def main():
    ...
    for p in files():
        ...
        for label,pattern in PATTERNS.items():
            checked=text
            if name.endswith('statistical-census-html/scripts/test_workflow.py') or name=='scripts/check_release.py':
    ...
    intermediate=json.loads((ROOT/'intermediates/export-manifest.json').read_text())
    for item in intermediate['files']:
        path=ROOT/'intermediates'/item['path']
        if hashlib.sha256(path.read_bytes()).hexdigest()!=item['export_sha256']:
            errors.append({'file':'intermediates/'+item['path'],'issue':'intermediate export hash changed'})
